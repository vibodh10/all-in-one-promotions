...
#[derive(Deserialize, Debug)]
struct BundleConfig {
    #[serde(rename = "minItems", alias = "min_items")]
...
#[derive(Debug)]
struct CartItem { line_id: String, product_id: String, quantity: i32, } #[shopify_function] fn cart_lines_discounts_generate_run( input: CartLinesDiscountsGenerateRunInput, ) -> Result<CartLinesDiscountsGenerateRunResult> {
    let offers: Vec<Offer> = match input.discount().metafield() {
        Some(m) => {
            match serde_json::from_str::<Vec<Offer>>(m.value()) {
                Ok(v) => v,
                Err(_) => {
                    match serde_json::from_str::<Offer>(m.value()) {
                        Ok(o) => vec![o],
                        Err(_) => vec![],
                    }
                }
